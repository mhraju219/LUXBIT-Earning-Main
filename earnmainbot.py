...
for var_name in ['BOT_TOKEN', 'CHANNEL_ID', 'WEBHOOK_URL']: if not globals()[var_name]: raise RuntimeError("Missing env var: {}".format(var_name))
...
cursor.execute(''' CREATE TABLE IF NOT EXISTS users ( user_id INTEGER PRIMARY KEY, balance REAL DEFAULT 0, wallet TEXT ) ''')
...
if BINANCE_API_KEY and BINANCE_API_SECRET and Client is not None: binance_client = Client(BINANCE_API_KEY, BINANCE_API_SECRET) logger.info("Binance API enabled") else: binance_client = None logger.warning("Binance API keys not set. Withdrawals are disabled.")
...
def set_user_wallet(user_id, wallet): cursor.execute("INSERT OR REPLACE INTO users (user_id, wallet) VALUES (?,?)", (user_id, wallet)) conn.commit()

def get_user_wallet(user_id): cursor.execute("SELECT wallet FROM users WHERE user_id=?", (user_id,)) row = cursor.fetchone() if row and row[0]: return row[0] return None
...
def fetch_withdrawal_proofs(): cursor.execute("SELECT user_id, amount, status, timestamp FROM withdrawals ORDER BY timestamp DESC LIMIT 5") rows = cursor.fetchall() if not rows: return "No withdrawal proofs yet." text = "📄 Recent withdrawal proofs:\n" for r in rows: text += "- User {}: {} units ({})\n".format(r[0], r[1], r[2]) return text

==============================

Binance Withdrawal (safe)

==============================

def process_binance_withdrawal(user_id, amount): if not binance_client: return False, "❌ Binance API not configured. Withdrawals are disabled." wallet = get_user_wallet(user_id) if not wallet: return False, "❌ No wallet set. Use /setwallet <address> first." try: result = binance_client.withdraw( coin='USDT', address=wallet, network='TRX', amount=amount ) return True, "✅ Withdrawal of {} USDT successful! TxID: {}".format(amount, result['id']) except Exception as e: logger.error(e) return False, "❌ Withdrawal failed: {}".format(str(e))
...
async def withdraw_command(update: Update, context: ContextTypes.DEFAULT_TYPE): user_id = update.from_user.id amount = ' '.join(context.args) if context.args else None if not amount or not amount.isdigit(): await update.message.reply_text("❌ Invalid amount. Usage: /withdraw <amount>") return amount = float(amount) balance = get_user_balance(user_id) if amount > balance: await update.message.reply_text("❌ Insufficient balance. Your balance: {}".format(balance)) return
...
async def setwallet_command(update: Update, context: ContextTypes.DEFAULT_TYPE): user_id = update.from_user.id wallet = ' '.join(context.args) if context.args else None if not wallet: await update.message.reply_text("❌ Invalid wallet. Usage: /setwallet <wallet_address>") return set_user_wallet(user_id, wallet) await update.message.reply_text("✅ Wallet address set: {}".format(wallet))
...
def main(): app = ApplicationBuilder().token(BOT_TOKEN).build() app.add_handler(CommandHandler('start', start)) app.add_handler(CommandHandler('withdraw', withdraw_command)) app.add_handler(CommandHandler('setwallet', setwallet_command)) app.add_handler(CallbackQueryHandler(button_handler))
